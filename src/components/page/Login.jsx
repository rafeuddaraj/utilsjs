...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    try {
      const user = await loginWithEmail(data); // Assuming loginWithEmail is an asynchronous operation
  ...
  const onClick = async (socialDetails) => {
    // console.log(socialDetails);
    try {
      const user = await loginWithSocial(socialDetails); // Assuming loginWithEmail is an asynchronous operation
      if (user.user) {
        navigate("/");
      }
      // navigate("/");
  ...
  return (
    <div className=" bg-gray-100 text-gray-900 flex justify-center">
      <div className="max-w-screen-xl m-0 sm:m-10 bg-white shadow sm:rounded-lg flex justify-center flex-1">
        ...
        <div
          className="lg:w-1/2 xl:w-5/12 h-full flex flex-col justify-center gap-20 p-6 sm:p-12"
          ref={ref}
          style={{
            zIndex: 0,
            transform: isInView ? "none" : "translateX(-200px)",
